...
LogThreat* logThread = new LogThreat();
std::mutex mtx; //dostęp do sekcji krytycznej przy kasowaniu wątku

struct thread_struct {
    int port; //port used for communication with client
    LogThreat::logNode* pointerOnLog;
...
*/
	...
	mtx.lock();
	logThread->saveServerLogs(logID);

	logThread->deleteThreat(logID);
	mtx.unlock();
...
void *save_log_thread(void * parm) {

    printf("Start Wątku Zapisu Logów \n");
    //int i=0;
    while(1){
    //i++;
    sleep(5);
...
int main() {
	...
	pthread_t save_log_thread_id;
	unsigned int len;
	len = sizeof(servaddr);
	//LogThreat* system = new LogThreat();


	pthread_create(&save_log_thread_id, NULL, save_log_thread, NULL);
